...
def do_organize(books: Sequence[Book], root: Path, cfg: Dict[str, Any],
    ...
    # pre-flight: two books resolving to the same destination folder would
    # merge or overwrite. Detect and abort before moving anything.
    dests: Dict[str, List[Path]] = {}
    for b in books:
        t = plan_layout(b, root, cfg)
        if t is not None:
            dests.setdefault(str(t).lower(), []).append(b.path)
    clash = {t: srcs for t, srcs in dests.items() if len(srcs) > 1}
    if clash:
        log.error("ABORTING organize: %d destination(s) would receive more than "
                  "one book. Nothing has been moved.", len(clash))
        for t, srcs in list(clash.items())[:10]:
            log.error("  %s  <-  %s", Path(t).name, ", ".join(s.name for s in srcs))
        return 0
...
if __name__ == "__main__":
    try:
        ...
        sys.stderr.write("\ninterrupted\n")
        sys.exit(130)#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
