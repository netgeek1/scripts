...
_VOL_TOKEN = re.compile(
    r"(?:^|[\s._#-])(?:book|bk|vol|volume|#)\s*0*(\d{1,3})\b"
    r"|(?:^|[\s._-])0*(\d{1,3})(?=[\s._-]|$)", re.I)


def _volume_of(name: str) -> Optional[int]:
    """A single-file book's volume number from its filename, if clear."""
    stem = Path(name).stem
    # a 'Part N'/'CD N' token is a divider within a book, not a volume number
    stem = re.sub(r"[\s._-]*\b(part|pt|cd|disc|disk)\s*\d+\b", " ", stem, flags=re.I)
    m = _VOL_TOKEN.search(stem)
    if not m:
        return None
...
def _looks_multipart(names: Sequence[str]) -> bool:
    """True when the files look like chapters/parts of ONE book.

    Chapters share a stable prefix and differ only by a running number, often
    with the word 'Chapter'/'Track'/'Part' or a NN-NN pattern.
    """
    stems = [Path(n).stem for n in names]
    if len(stems) < 2:
        return True
    chaptery = sum(1 for s in stems
                   if re.search(r"\b(chapter|track|part|episode|section|"
                                r"prologue|epilogue|credits)\b", s, re.I))
    if chaptery >= max(2, len(stems) * 0.5):
        return True
    # common leading prefix of at least ~8 chars means one titled work
    prefix = os.path.commonprefix(stems)
    if len(prefix) >= 8 and re.search(r"\d", prefix) is None:
        # they share a title and differ only by trailing number -> chapters
        tails = {s[len(prefix):] for s in stems}
...
def split_distinct_books(files: Sequence[Path]) -> List[List[Path]]:
    """Divide files in one directory into separate books when they clearly are.

    A folder holding several complete single-file books (an author dumping
    'Series 1.m4b', 'Series 2.m4b', ...) must NOT be treated as one multi-file
    book, or they would be renumbered together and renamed onto each other.

    The tricky part: distinct books in a series share a long title prefix, and
    so do chapters of one book. The distinguishing signals:
      - chapters carry Chapter/Track/Part/Epilogue wording, or a NN-NN pattern
      - distinct books each carry a DIFFERENT whole-book volume number and
        (usually) 'Book N' in the name
    Volume-number distinctness is checked BEFORE the shared-prefix test, so a
    series prefix does not mask that these are separate books.
    """
    files = list(files)
    if len(files) < 2:
        return [files]
    ...
    # Strong chapter signal: explicit chapter/part wording on most files.
    chaptery = sum(1 for st in stems
    ...
    # 'Part N' / 'Pt N' / 'CD N' is a within-book divider. If removing that
    # token leaves the same base for every file, it is one book in parts.
    part_re = re.compile(r"[\s._-]*\b(part|pt|cd|disc|disk)\s*\d+\b", re.I)
    ...
    distinct = {v for v in vols if v is not None}
    have_book_word = sum(1 for st in stems
                         if re.search(r"\b(book|vol|volume)\b", st, re.I))
    all_single_type = all(f.suffix.lower() in (".m4b", ".m4a") for f in files)
    if len(distinct) >= 2 and (
            len(distinct) == len([v for v in vols if v is not None])) and (
            have_book_word or all_single_type):
        by_vol: Dict[Any, List[Path]] = {}
        for f, v in zip(files, vols):
            by_vol.setdefault(v if v is not None else f.name, []).append(f)
        return [sorted(g, key=natural_key)
                for _, g in sorted(by_vol.items(),
                                   key=lambda kv: (kv[0] is None, kv[0]))]

    # Otherwise assume one book (chapters sharing a prefix and a running index).
    if _looks_multipart(names):
        return [files]
    return [files]
...
def _organize_base(scan_root: Path, books: Sequence[Book],
    ...
    """Where the Author/Series/Book tree is rooted.

    Always the library the user pointed at - never a book's own folder, and
    never a climb above the scan root (that once produced C:\\). The only
    special case is being pointed at a single dedicated book folder, where the
    tree roots at that folder's parent so it does not nest inside itself.
    """
    ...
    if configured:
        ...
        if lib.is_dir() and (lib == scan_root or lib in scan_root.parents):
            base = lib
        else:
            base = scan_root
    else:
    ...
    # pointed at ONE dedicated book folder: root the tree one level up
    if len(books) == 1 and book_dirs[0] == base:
        parent = base.parent
        if parent and parent != base:
            log.info("organize: target is a single book folder; rooting the "
                     "moved tree at its parent %s", parent)
            return parent
    return base
...
def do_organize(books: Sequence[Book], root: Path, cfg: Dict[str, Any],
    ...
    # how many books live directly in each folder - a folder shared by several
    # books (loose single-file books) cannot be moved as a unit
    ...
    for b in books:
        ...
        if b.match_score is not None and b.match_score < 100 and not m.asin:
            held.append(b.path.name)
            log.warning("uncertain metadata, not moving: %s (score %.0f)",
                        b.path.name, b.match_score if b.match_score else 0)
            continue
        ...
        if dest is None:
            ...
            continue
        dest = dest.resolve()
        src_dir = b.path.resolve()

        # a folder is movable as a unit only if this is the sole book in it and
        # it is neither the scan root, the base, nor an ancestor of the base
        dedicated = (path_counts[src_dir] == 1 and src_dir != scan_root
                     and src_dir != base and src_dir not in base.parents)
    ...
    # Show the COMPLETE plan - every book and what would happen to it - before
    # anything is written, on dry runs AND real runs. A real run then asks
    # whether to proceed (only when there are conflicts to resolve).
    prefix = "[dry-run] " if dry_run else ""
    log.info("%sorganize plan:", prefix)
    for b, dest, move_folder in sorted(plan, key=lambda t: str(t[1]).lower()):
        if str(dest).lower() in clash_keys:
            log.warning("  %sCONFLICT  %s -> %s  (shares destination)",
                        prefix, b.path.name, dest)
        elif move_folder:
            log.info("  %smove      %s -> %s", prefix, b.path.name, dest)
        else:
            log.info("  %smove %d file(s)  %r -> %s",
                     prefix, len(b.files), (b.final.title or b.path.name), dest)
    for name in sorted(already):
        log.info("  %sin place  %s", prefix, name)
    for name in sorted(held):
        log.info("  %sheld      %s  (uncertain metadata)", prefix, name)
    for name in sorted(undetermined):
        log.info("  %sno dest   %s", prefix, name)
    _breakdown()
    ...
    if clash_keys:
        ...
        # no conflicts: still confirm before a real move
        try:
            if input(f"  Move {len(plan)} book(s)? [y/N] ").strip().lower() \
                    not in ("y", "yes"):
                log.warning("organize: nothing moved.")
                return 0
        except (EOFError, KeyboardInterrupt):
    ...
    # plan already shown and confirmed; execute quietly, reporting only trouble
    moved = 0
    for b, dest, move_folder in plan:
        if move_folder:
            ...
            for f in b.files:
                tgt = dest / f.name
                if str(tgt).lower() in file_targets or tgt.exists():
                    log.error("  would overwrite %s - skipping %r",
                              tgt.name, (b.final.title or b.path.name))
                    collision = True
                    break
                file_targets[f] = tgt
            if collision:
            ...
            for f, tgt in file_targets.items():
...
def do_rename(books: Sequence[Book], cfg: Dict[str, Any], dry_run: bool,
    ...
    # Plan every rename first, then refuse to execute if two sources would land
    # on the same target - renaming three books onto 'Book 1.m4b' destroys two
    # of them. A collision means the books were mis-grouped; stop and report.
    ...
    for b in books:
        ...
        for idx, f in enumerate(files, start=1):
            stem = render_template(template, template_fields(b, index=idx, total=total))
            if not stem:
                continue
    ...
    # collision detection across the WHOLE run
    ...
    # Show the full plan first, on dry runs AND real runs.
    prefix = "[dry-run] " if dry_run else ""
    log.info("%srename plan:", prefix)
    for src, dst in sorted(plan, key=lambda t: str(t[1]).lower()):
        if str(dst).lower() in clash_keys:
            log.warning("  %sCONFLICT %s -> %s  (shares target)",
                        prefix, src.name, dst.name)
        elif dst.exists():
            log.warning("  %sskip     %s: %s already exists",
                        prefix, src.name, dst.name)
        else:
            log.info("  %srename   %s -> %s", prefix, src.name, dst.name)
    _rename_breakdown()
    ...
    if clash_keys:
        ...
        try:
            if input(f"  Rename {len(plan)} file(s)? [y/N] ").strip().lower() \
                    not in ("y", "yes"):
                log.warning("rename: nothing renamed.")
                return 0
        except (EOFError, KeyboardInterrupt):
            log.warning("rename: nothing renamed.")
            return 0
