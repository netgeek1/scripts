...
**Multiple books in one folder are detected and kept separate.** Grouping uses
the embedded **album tag** as the per-book identity: files that share an album
are one book's chapters; files with different albums are separate books. This
holds even after `rename` reduces filenames to bare titles. When files carry no
