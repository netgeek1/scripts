...
**Point `organize` at the library root, not a single book folder.** It builds
`Author/Series/Book NN - Title` under the root you give it, so if that root
*is* one book, the tree would nest inside it. The tool detects this, re-roots
at the library (or the parent), and warns; but the intended usage is the whole
library or an author folder.
...
or OpenAudible.# audiobook-tagger
