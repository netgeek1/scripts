...
hl_fac = s.get("headerless_facility")     # e.g. syslog -> tag messages with no PRI/header
hl_pri = s.get("headerless_priority")     # e.g. emerg
hldef = ""
if hl_fac:
    hldef += " default-facility(%s)" % hl_fac
if hl_pri:
    hldef += " default-priority(%s)" % hl_pri
o = []
...
nets = []
if udp:
    nets.append('    network(transport("udp") port(%d)%s);' % (udp, hldef))
if tcp:
    nets.append('    network(transport("tcp") port(%d)%s);' % (tcp, hldef))
if nets:
...
if asbool(s.get("archive_enable"), True):
    ...
    if a_perm:
        opts.append('    perm(%s)' % a_perm)
    if a_dperm:
        opts.append('    dir-perm(%s)' % a_dperm)
    if a_owner:
        opts.append('    owner("%s")' % a_owner)
    if a_group:
        opts.append('    group("%s")' % a_group)
    if a_fifo:
        opts.append('    log-fifo-size(%s)' % a_fifo)
    opts.append('    flush-lines(0)')
    o.append("# Archive everything received over the network.")
    o.append("destination d_net_archive {")
    o.append('    file("%s/%s"' % (a_root, a_sub))
    o.append("\n".join(opts) + " );")
    o.append("};")
    netsrcs = []
    if nets:
...
# Generate /usr/local/bin/alert-logmaint.sh + cron: the loglinks (today/yesterday
# symlinks + day-complete banner) and logclean (gzip + retention) behavior,
# parameterized from settings. Values are baked in at generation time, so re-run
# install/regen after changing archive_* settings.
write_logmaint() {
  ...
  {
    echo '#!/bin/sh'
    echo '# Generated by syslog-alert-router.sh -- do not edit (regenerate via install/regen).'
    echo "ROOT=\"$root\""
    echo "COMPRESS_AFTER=$comp"
    echo "DELETE_AFTER=$del"
    echo "SYMLINKS=$syms"
    cat <<'LOGMAINT_BODY'
case "$1" in
  links)
    [ "$SYMLINKS" = true ] || exit 0
    Y=$(date +%Y); M=$(date +%m); D=$(date +%d)
    rm -f "$ROOT/yesterday"
    [ -e "$ROOT/today" ] && mv "$ROOT/today" "$ROOT/yesterday"
    ln -sfn "$ROOT/$Y/$M/$D" "$ROOT/today"
    if [ -d "$ROOT/yesterday" ]; then
      find -L "$ROOT/yesterday" -type f -name '*.log' 2>/dev/null | while IFS= read -r f; do
        { echo "*******************************************************************"
          echo "*** DAY COMPLETE -- CHECK FOR NEW LOG FILES in $ROOT/$Y/$M/$D ***"
          echo "*******************************************************************"
        } >> "$f"
      done
    fi
    ;;
  clean)
    find "$ROOT" -type f -name '*.log'    -mtime +$COMPRESS_AFTER -exec gzip {} \; 2>/dev/null
    find "$ROOT" -type f -name '*.log.gz' -mtime +$DELETE_AFTER   -exec rm -f {} \; 2>/dev/null
    ;;
  *) echo "usage: $0 {links|clean}" >&2; exit 2;;
esac
LOGMAINT_BODY
  } | atomic_write /usr/local/bin/alert-logmaint.sh 0755
  atomic_write /etc/cron.d/alert-logmaint 0644 <<'CRON'
# Generated by syslog-alert-router.sh
SHELL=/bin/sh
PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
5  0 * * * root /usr/local/bin/alert-logmaint.sh links
30 0 * * * root /usr/local/bin/alert-logmaint.sh clean
CRON
}
...
def cmd_edit(a):
    """Merge-update an existing relay: only provided fields change; rest kept."""
    doc = _load()
    rel = doc["relays"]["defs"].get(a.name)
    if rel is None:
        print("no such relay: %s (use 'add' to create it)" % a.name, file=sys.stderr)
        return 1
    rel = dict(rel)
    if a.transport:
        rel["transport"] = a.transport
    ...
    if a.security is not None:
        rel["security"] = a.security
    if a.from_addr is not None:
        rel.pop("from", None) if a.from_addr == "" else rel.__setitem__("from", a.from_addr)
    if a.sendmail is not None:
        rel["sendmail"] = a.sendmail
    if a.user is not None:
        rel["user"] = a.user
    if a.secret_file is not None:
        rel["secret_file"] = a.secret_file; rel.pop("secret_cmd", None)
    if a.secret_cmd is not None:
        rel["secret_cmd"] = a.secret_cmd; rel.pop("secret_file", None)
    if a.tls_verify == "on":
        rel.pop("tls_verify", None)
    elif a.tls_verify == "off":
        rel["tls_verify"] = False
    if a.auth == "on":
        rel["auth"] = True
        if not rel.get("user"):
            print("auth on but no user set; pass --user", file=sys.stderr)
            return 2
        if not (rel.get("secret_file") or rel.get("secret_cmd")):
            print("auth on but no secret_file/secret_cmd (set a password first)", file=sys.stderr)
            return 2
    elif a.auth == "off":
        rel["auth"] = False
    if str(rel.get("transport", "smtp")) == "smtp" and not rel.get("host"):
        print("smtp relay needs a host", file=sys.stderr)
        return 2
    doc["relays"]["defs"][a.name] = rel
    _save(doc)
    print("updated relay %s" % a.name)
    return 0
...
def main(argv=None):
    ...
    sp.add_argument("--name", required=True)
    sp.add_argument("--transport", choices=["smtp", "sendmail"])
    sp.add_argument("--host"); sp.add_argument("--port")
    sp.add_argument("--security", choices=["none", "starttls", "tls"])
    sp.add_argument("--from", dest="from_addr")
    sp.add_argument("--auth", choices=["on", "off"])
    sp.add_argument("--user")
    sp.add_argument("--secret-file", dest="secret_file")
    sp.add_argument("--secret-cmd", dest="secret_cmd")
    sp.add_argument("--tls-verify", choices=["on", "off"])
    sp.add_argument("--sendmail")
    ...
    return {
        ...
        "del": cmd_del, "set-order": cmd_set_order, "check": cmd_check,
    }[a.cmd](a)
...
_menu_edit_relay() {
  local name host port sec frm au user args
  printf 'relay name to edit> '; read -r name
  [ -n "$name" ] || { echo "cancelled"; return 0; }
  python3 "$ALERTRELAYS" show "$name" >/dev/null 2>&1 || { echo "no such relay: $name"; return 0; }
  echo "Current definition:"; python3 "$ALERTRELAYS" show "$name" | sed 's/^/  /'
  echo "New values (blank = keep current):"
  printf 'host> '; read -r host
  printf 'port> '; read -r port
  printf 'security [none/starttls/tls]> '; read -r sec
  printf 'envelope from ("-" to clear)> '; read -r frm
  printf 'requires auth? [on/off, blank = keep]> '; read -r au
  printf 'username (for auth)> '; read -r user
  args=(edit --name "$name")
  [ -n "$host" ] && args+=(--host "$host")
  [ -n "$port" ] && args+=(--port "$port")
  [ -n "$sec" ]  && args+=(--security "$sec")
  if [ -n "$frm" ]; then [ "$frm" = "-" ] && args+=(--from "") || args+=(--from "$frm"); fi
  [ -n "$user" ] && args+=(--user "$user")
  case "$au" in
    on|On|y|Y|yes)
      if [ ! -s "$SECRETS/$name.pw" ]; then
        echo "Auth needs a password (none stored yet):"
        local sf; sf="$(write_secret "$name")" || { echo "password not set; aborting edit"; return 0; }
        args+=(--secret-file "$sf")
      fi
      args+=(--auth on);;
    off|Off|n|N|no) args+=(--auth off);;
  esac
  python3 "$ALERTRELAYS" "${args[@]}"
}
...
cmd_relays() {
  ...
  while true; do
    ...
    python3 "$ALERTRELAYS" list
    echo "-----------------------------------------------"
    echo "  a) add relay      e) edit relay"
    echo "  o) set order      p) set password"
    echo "  t) test relay     d) delete relay"
    echo "  q) quit"
    printf 'choose> '; read -r choice
    case "$choice" in
      a|A) _menu_add_relay || true;;
      e|E) _menu_edit_relay || true;;
...
cmd_uninstall() {
  ...
  backup "$FRAGMENT"; rm -f -- "$FRAGMENT" "$CRON" "$DISPATCH" "$SWEEPER" "$ALERTRELAYS" "$ALERTRULES" "$ALERTLIB" \
    /usr/local/bin/alert-logmaint.sh /etc/cron.d/alert-logmaint
